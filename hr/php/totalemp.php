<?php
    ...
    header('Content-type: application/json'); 
        ...
        $result = mysqli_query($con, "SELECT count(*) FROM account WHERE office = 'HR'");
        $num_rows = mysqli_fetch_array($result);
        $total = $num_rows[0];
        $obj['hr'] = 'Number of employees in HR Office are: ' . $total;
        
        $result2 = mysqli_query($con, "SELECT count(*) FROM account WHERE office = 'Supply'");
        $num_rows2 = mysqli_fetch_array($result2);
        $total2 = $num_rows2[0];
        $obj['supply'] = 'Number of employees in Supply Office are: ' . $total2;

        $result3 = mysqli_query($con, "SELECT count(*) FROM account WHERE office = 'Accounting'");
